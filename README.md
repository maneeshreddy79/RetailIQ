...
The current V0.7 implementation does **not use machine-learning models for prediction**.
...
RetailIQ now includes an adaptive ML analysis tab with:

- automatic target/task suggestions;
- classification baselines: Logistic Regression, Decision Tree and Random Forest;
- regression baselines: Linear Regression, Decision Tree and Random Forest;
- hold-out evaluation with task-appropriate metrics;
- feature-importance/coefficient views where supported;
- K-Means clustering with Silhouette Score;
- Isolation Forest anomaly detection.

See `docs/ml_methodology.md` for the research methodology and evaluation design.


## V0.8 Research Evaluation
V1.0 adds a reproducible 12-dataset research suite with leakage-safe 5×5 repeated cross-validation, Gradient Boosting comparisons, ground-truth-aware anomaly evaluation, chronological forecasting validation, K-Means K-range evaluation, runtime logging, and multiple-comparison-corrected statistical tests. Primary results are saved under `results/research_suite/`.

## 🤖 V1.0 ML and Research Extensions

RetailIQ V1.0 adds seven distinct analytical ML algorithms/paths:
...
- K-Means clustering
- Isolation Forest anomaly detection
- Ridge lag forecasting (with Gradient Boosting lag forecasting as a second forecasting baseline)

The system supports classification, regression, unsupervised clustering, anomaly detection, and time-series forecasting. Reference/ground-truth anomaly labels are excluded from unsupervised model inputs and used only for independent evaluation when available.

## 🔬 V1.0 Research Protocol

The primary research suite contains 12 datasets:

- 5 classification
- 2 regression
- 3 unsupervised
- 1 anomaly-detection benchmark with ground truth
- 1 temporal forecasting benchmark

General supervised benchmarks use 5 repeats × 5 folds with preprocessing fitted within each training fold. Classification uses weighted F1 as the primary metric; regression uses RMSE. Temporal data uses chronological validation rather than random future/past mixing. Statistical comparisons use Friedman tests followed by Wilcoxon signed-rank tests with Holm correction when appropriate.

The primary research artifacts are stored in `results/research_suite/`.
