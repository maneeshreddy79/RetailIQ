...
def render_regression(df, profile, mode, selected_target, resolved_task):
    """Render the Regression page using existing supervised backend."""
    ...
    page_header(
        "Regression",
        "Train and compare regression models. Select a continuous target and run the benchmark.",
        "cyan",
        mode,
    )
    ...
    selected_target = st.selectbox("Target column", target_options, index=default_index, key="ml_target_reg")

    if selected_target == "— No target / unsupervised —":
        empty_state("📈", "Select a supervised target above to enable regression analysis.", mode)
        return selected_target, None

    inferred = infer_task(df[selected_target])
    if inferred != "regression":
        st.warning(f"Detected task for `{selected_target}` is **{inferred}**, not regression. This target may be better suited for the Classification page.")
        return selected_target, inferred

    task_choice = st.selectbox("ML task", ["Auto", "classification", "regression"], index=0, key="ml_task_reg")
    resolved_task = inferred if task_choice == "Auto" else task_choice
    st.info(f"Detected task: **{inferred}** · Selected task: **{resolved_task}**")

    test_size = st.slider("Test-set size", 0.15, 0.40, 0.20, 0.05, key="ml_test_size_reg")

    if st.button("Run Regression Benchmark", type="primary", key="run_regression"):
        with st.spinner("Training baseline models and evaluating the hold-out test set..."):
            ml_result = run_supervised(df, selected_target, "regression", test_size=test_size)
        ...
        if not valid.empty and "RMSE" in valid.columns:
        ...
        try:
            ...
            st.warning(f"Experiment logging failed: {exc}")
        ...
        if model_names:
            chosen_model = st.selectbox("Model explanation", model_names, key="ml_explain_model_reg")
